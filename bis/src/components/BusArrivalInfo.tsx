...
import { BusArrival, BusRoute } from "../types";
...
const BusArrivalInfo: React.FC<BusArrivalInfoProps> = ({
  ...
  const uniqueArrivals = mockArrivals.reduce((unique: BusArrival[], bus) => {
    ...
    if (!existingBus || bus.arrivalTime < existingBus.arrivalTime) {
      return unique
        .filter((b) => b.routeNumber !== bus.routeNumber)
        .concat(bus);
    }
    return unique;
  }, []);
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {Array.from({ length: Math.ceil(uniqueArrivals.length / 4) }).map(
            (_, slideIndex) => (
              <div
                key={slideIndex}
                style={{
                          ...
                          <div style={{ textAlign: "right" }}>
                            <div
                              style={{
                                ...
                                fontWeight: "700",
                                color:
                                  bus.arrivalTime <= 3 ? "#dc2626" : "#059669",
                                marginBottom: "2px",
                              }}
                            >
