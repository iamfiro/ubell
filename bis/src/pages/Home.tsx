...
function Home() {
  ...
  useEffect(() => {
    if (uniqueBuses && uniqueBuses.length > 0) {
      ...
      uniqueBuses.forEach((bus, idx) => {
        console.log(`  [${idx}] ${bus.routeNo}번 - ${Math.ceil(bus.arrivalTime)}분`);
      });
    }
  }, [uniqueBuses]);
  ...
  const convertNumbersToKorean = useCallback((text: string) => {
    ...
    return text.replace(/(\d+)([A-Z]?)(-(\d+))?번/g, (_, mainNum, alpha, __, subNum) => {
  ...
  const speakWithElevenLabs = useCallback(async (text: string) => {
    ...
    console.log(`🎛️ 오디오 객체 전환: ${useAudioRef1.current ? 'audioRef2 → audioRef1' : 'audioRef1 → audioRef2'}`);
    ...
    try {
      ...
      const response = await fetch('https://api.elevenlabs.io/v1/text-to-speech/uyVNoMrnUku1dZyVEXwD', {
        method: 'POST',
        headers: {
          'Accept': 'audio/mpeg',
  ...
  const startVoiceMode = useCallback(() => {
    ...
    console.log(`🎯 음성 모드 시작: focusedBusIndex=${focusedBusIndex}, uniqueBuses 길이=${uniqueBuses?.length || 0}`);
    if (uniqueBuses && uniqueBuses[focusedBusIndex]) {
      console.log(`🚌 포커스된 버스: [${focusedBusIndex}] ${uniqueBuses[focusedBusIndex].routeNo}번`);
    }
    setIsVoiceMode(true);
    setSelectedBusIndex(focusedBusIndex);
    setIsFirstVoiceInput(true); // 첫 입력 플래그 설정
    
    speak("음성 모드입니다. 플러스, 마이너스 버튼으로 버스를 선택하고 엔터키로 호출하세요.");
    resetInputTimer();
  ...
  const speakCurrentBus = useCallback((targetIndex?: number) => {
    const busIndex = targetIndex !== undefined ? targetIndex : selectedBusIndex;
    console.log(`🎤 speakCurrentBus 호출 - targetIndex: ${targetIndex}, busIndex: ${busIndex}, selectedBusIndex: ${selectedBusIndex}, uniqueBuses 길이: ${uniqueBuses?.length || 0}`);
    ...
    if (busIndex < 0 || busIndex >= uniqueBuses.length) {
      console.error(`❌ 잘못된 busIndex: ${busIndex}, 가능한 범위: 0-${uniqueBuses.length - 1}`);
      return;
    }

    const currentBus = uniqueBuses[busIndex];
    console.log(`🚌 선택된 버스 [인덱스 ${busIndex}]: ${currentBus?.routeNo}번 (arrivalTime: ${currentBus?.arrivalTime}초)`);
    console.log(`🚌 전체 uniqueBuses 배열:`, uniqueBuses.map((bus, idx) => `[${idx}]${bus.routeNo}번`));
    console.log(`🚌 현재 버스 상세:`, currentBus);
    
    if (currentBus) {
      const isCalled = calledBuses.has(currentBus.routeNo.toLowerCase());
      if (isCalled) {
        speak(`${currentBus.routeNo}번 호출되었습니다. 취소하시려면 엔터키를 눌러주세요`);
      } else {
        console.log('현재 선택된 버스: ', currentBus.routeNo, currentBus.arrivalTime);
  ...
  const selectNextBus = useCallback(() => {
    if (uniqueBuses && uniqueBuses.length > 0) {
      if (isFirstVoiceInput) {
        ...
        console.log(`➡️ 다음 버스 선택: ${selectedBusIndex}(${currentBus?.routeNo}번) → ${nextIndex}(${nextBus?.routeNo}번)`);
  ...
  const selectPrevBus = useCallback(() => {
    if (uniqueBuses && uniqueBuses.length > 0) {
      if (isFirstVoiceInput) {
        ...
        // 일반적인 이전 버스 선택
        const prevIndex = selectedBusIndex === 0 ? uniqueBuses.length - 1 : selectedBusIndex - 1;
        const currentBus = uniqueBuses[selectedBusIndex];
        const prevBus = uniqueBuses[prevIndex];
        console.log(`⬅️ 이전 버스 선택: ${selectedBusIndex}(${currentBus?.routeNo}번) → ${prevIndex}(${prevBus?.routeNo}번)`);
  ...
  const focusNextBus = useCallback(() => {
    if (uniqueBuses && uniqueBuses.length > 0) {
      ...
      console.log(`🔍 포커스 다음: ${focusedBusIndex}(${uniqueBuses[focusedBusIndex]?.routeNo}번) → ${nextIndex}(${uniqueBuses[nextIndex]?.routeNo}번) (총 ${uniqueBuses.length}개)`);
      setFocusedBusIndex(nextIndex);
    }
  }, [uniqueBuses, focusedBusIndex, isFocusMode, startFocusMode, resetFocusTimer]);
  ...
  const focusPrevBus = useCallback(() => {
    if (uniqueBuses && uniqueBuses.length > 0) {
      ...
      const prevIndex = focusedBusIndex === 0 ? uniqueBuses.length - 1 : focusedBusIndex - 1;
      console.log(`🔍 포커스 이전: ${focusedBusIndex}(${uniqueBuses[focusedBusIndex]?.routeNo}번) → ${prevIndex}(${uniqueBuses[prevIndex]?.routeNo}번) (총 ${uniqueBuses.length}개)`);
  ...
  }, [isFocusMode, uniqueBuses, focusedBusIndex, calledBuses, removeBusCall, handleBusCall, resetFocusTimer]);
          ...
          if (uniqueBuses && uniqueBuses[selectedBusIndex]) {
            if (isFirstVoiceInput) {
              ...
              if (isCalled) {
                ...
                // 버스 호출
                handleBusCall(selectedBus.routeNo);
                const arrivalTimeInMinutes = Math.ceil(selectedBus.arrivalTime / 60);
                const arrivalTime = arrivalTimeInMinutes <= 0 ? '곧' : `${arrivalTimeInMinutes}분`;
                speak(`${selectedBus.routeNo}번 버스 호출되었습니다. 도착까지 ${arrivalTime} 남았습니다.`);
              }
            }
          }
          break;
  ...
  }, [isVoiceMode, isFocusMode, startVoiceMode, resetInputTimer, resetFocusTimer, selectNextBus, selectPrevBus, focusNextBus, focusPrevBus, handleFocusedBusAction, uniqueBuses, selectedBusIndex, calledBuses, handleBusCall, removeBusCall, speak, exitVoiceMode, isFirstVoiceInput]);
      ...
      >
        ...
        {!isVoiceMode && (
          <div style={{
            ...
            키패드 [0]번: 음성 안내 모드 | [+][-]: 포커스 선택 (3초 자동해제-테스트) | [Enter]: 호출/취소
          </div>
        )}
